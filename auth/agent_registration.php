...
// Get agent type from URL (e.g., direct_agent or associate_agent)
$agent_type_param = $_GET['type'] ?? '';

// Get error message from URL, safely encoded for output
    ...
    <form action="../public/api/agent_registration_complete.php" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <select id="user_type" name="user_type" required>
                ...
                <option value="direct_agent" <?= $agent_type_param === 'direct_agent' ? 'selected' : '' ?>>Direct Agent</option>
                <option value="associate_agent" <?= $agent_type_param === 'associate_agent' ? 'selected' : '' ?>>Associate Agent</option>
            </select>
        </div>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="text" id="broker_id" name="broker_id" value="<?= htmlspecialchars($user['broker_id'] ?? '') ?>">
            </div>
            <div class="form-group">
                <label for="prc_number">PRC Number</label>
                <input type="text" id="prc_number" name="prc_number" value="<?= htmlspecialchars($user['prc_number'] ?? '') ?>">
            </div>
        </div>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <select id="experience_years" name="experience_years">
                    ...
                    foreach($exp_options as $exp) {
                        $selected = ($user['experience_years'] ?? '') === $exp ? 'selected' : '';
            ...
            <div class="form-group">
                ...
                <input type="text" id="specializations" name="specializations"
                       value="<?= htmlspecialchars($user['specializations'] ?? '') ?>" placeholder="e.g., Residential, Commercial, Luxury">
            </div>
        </div>

        <div class="form-group">
            <label for="experience_details">Experience Details</label>
            <textarea id="experience_details" name="experience_details" rows="4" placeholder="Describe your real estate experience and achievements"><?= htmlspecialchars($user['experience_details'] ?? '') ?></textarea>
        </div>
            ...
            <div class="form-group">
                ...
                <select id="education" name="education">
                    ...
                    foreach($edu_levels as $edu) {
                        $selected = ($user['education'] ?? '') === $edu ? 'selected' : '';
        ...
        <div class="form-row">
            ...
            <div class="form-group">
                ...
                <input type="number" id="graduation_year" name="graduation_year" min="1950" max="2030" value="<?= htmlspecialchars($user['graduation_year'] ?? '') ?>">
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <textarea id="certifications" name="certifications" rows="3"><?= htmlspecialchars($user['certifications'] ?? '') ?></textarea>
        </div>
        <div class="form-group">
            <label for="training">Additional Training</label>
            <textarea id="training" name="training" rows="3"><?= htmlspecialchars($user['training'] ?? '') ?></textarea>
        </div>
            ...
            <select name="company_id" id="company_id" class="form-control">
                ...
                try {
                    ...
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        $selected = ($user['company_id'] ?? '') == $row['id'] ? 'selected' : '';
