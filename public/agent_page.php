<?php
  ...
  // --- Agent Display Info with Default Avatar on 404 ---
  ...
  if (!empty($agent['profile_image_path'])) {
      // Build server path to check if file exists
  ...
  $properties = [];
  $agent_user_id = $agent['id']; // This is the user_id from users table
  $user_type     = $agent['user_type'] ?? 'associate'; // default fallback

  if ($user_type === 'associate') {
      // Associate: created by agent (unsold) OR sold by agent
      $stmt = $conn->prepare("
          SELECT p.*, s.first_name AS sold_by_first_name, s.last_name AS sold_by_last_name
          FROM properties p
          LEFT JOIN users s ON p.sold_by_agent_id = s.id
          WHERE (p.agent_id = ? AND p.sold_by_agent_id IS NULL)
            OR (p.agent_id != ? AND p.sold_by_agent_id = ?)
          ORDER BY p.created_at DESC
      ");
      $stmt->bind_param("iii", $agent_user_id, $agent_user_id, $agent_user_id);
  } else {
      // Direct: show all properties created by this agent
      $stmt = $conn->prepare("
          SELECT p.*, s.first_name AS sold_by_first_name, s.last_name AS sold_by_last_name
          FROM properties p
          LEFT JOIN users s ON p.sold_by_agent_id = s.id
