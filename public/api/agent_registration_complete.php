<?php
    ...
    try {
        ...
        // Only require professional info if not a Direct Agent
        if (isset($_POST['user_type']) && $_POST['user_type'] !== 'direct_agent') {
            $required_fields[] = 'experience_years';
        }

        foreach ($required_fields as $f) {
            if (empty($old_inputs[$f])) {
                throw new Exception("The field '$f' is required.");
            }
        }
        ...
        elseif ($old_inputs['user_type'] === 'direct_agent') {
            ...
            // Clear professional info fields for Direct Agents
            $broker_license = null;
            $prc_license = null;
            $resume = null;
