...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // === Registration Notices Modal Display ===
        (async () => {
            try {
                const res = await fetch("/BatEstateExplorer/public/api/check_result.php", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({})
                });
                ...
                const unseenNotices = data.notices.filter(n => !n.is_seen);
                if (!unseenNotices.length) return;
                ...
                const showNextNotice = async () => {
                    ...
                    switch (notice.notice_type) {
                        case "approve":
                            ...
                            iconSvg = `
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" width="72" height="72" fill="none" stroke="${accentColor}" stroke-width="2.5">
                                    <circle cx="32" cy="32" r="28" fill="${iconBg}" stroke="${accentColor}"/>
                                    <path d="M20 34 L28 42 L44 26" stroke="${accentColor}" stroke-width="4" stroke-linecap="round"/>
                                </svg>`;
                            break;
                        case "reject":
                            accentColor = "#ef4444";
                    ...
                    modal.querySelector(".btn").addEventListener("click", async () => {
                        ...
                        try {
                            const markSeenRes = await fetch("/BatEstateExplorer/public/api/mark_notice_seen.php", {
                                method: "POST",
                                headers: { "Content-Type": "application/json" },
                                body: JSON.stringify({ id: notice.notice_id })
                            });

                            const markSeenData = await markSeenRes.json().catch(() => ({}));
                            if (!markSeenData.success) console.error("Failed to mark notice as seen:", markSeenData);
                ...
                console.error("Failed to load registration notices:", err);
            }
        })();
