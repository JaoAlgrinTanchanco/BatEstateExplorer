...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const API_BASE = '/BatEstateExplorer/public/api/'; // Absolute base path
    ...
    saveDraftBtn.addEventListener('click', async () => {
        ...
        // Append uploaded image filenames
        const imageNames = Array.from(imageInput.files).map(file => file.name);
        formData.append('images', JSON.stringify(imageNames));

        try {
            const res = await fetch(API_BASE + 'save_draft.php', { method: 'POST', body: formData });
    ...
    async function loadDrafts() {
        try {
            ...
            drafts.forEach(d => {
                ...
                card.innerHTML = `
                    <span class="delete-draft" title="Delete Draft">&times;</span>
                    <strong>${d.title}</strong>
                    <p><small>${new Date(d.created_at).toLocaleString()}</small></p>
                    <button type="button" class="load-draft-btn" data-id="${d.id}">Load</button>
                `;
    ...
    function attachDraftEvents() {
        ...
        draftContainer.querySelectorAll('.delete-draft').forEach(btn => {
            btn.addEventListener('click', async () => {
                const draftCard = btn.closest('.draft-card');
